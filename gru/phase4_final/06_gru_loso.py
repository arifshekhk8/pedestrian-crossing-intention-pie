"""06_gru_loso.py — GRU study Phase G4: 6-fold Leave-One-Set-Out CV (generalization check).
...
(so the held-out AUC is directly comparable to the BiLSTM's LOSO 0.928 and the transformer's
0.939 — all AUC-selected).

TRAINING is done by the unified engine's train_run (family gru, the F1-winner config) — NO loop
is duplicated. The engine ignores test, so the held-out set is scored separately from the saved
checkpoint. Seed 42. CPU.
...
Run from the repo root:  python gru/phase4_final/06_gru_loso.py
"""
import csv
...
def evaluate(run_dir, X, y):
    ...
    model = engine.MODEL_REGISTRY["gru"](F1_WINNER_CFG)
...
def fold(test_set, X, y, sid, pid, seed=42):
    ...
    if not ((run_dir / "best.pt").exists()):
        ...
        engine.train_run("gru", F1_WINNER_CFG, seed, CPU,
...
def main():
    ...
    print(f"LOSO on gru_f1_winner {F1_WINNER_CFG} | seed 42 | CPU\n")
