"""05_gru_test_eval.py — GRU study Phase G4: THE single designated test-touch.

This is the ONLY script in the GRU study that reads test set03. It runs exactly once, after
the G3 human checkpoint cleared. Mirrors f1_optimization/05_final_test_eval.py's discipline:

  1. PARITY GATE — regenerate the frozen BiLSTM's per-seed test AUC from its checkpoints and
     assert it matches the stored final.json values (0.9131...) BEFORE any GRU number is
     emitted. If the frozen baseline has drifted, abort.
  2. For each GRU arm (from runs_final/), regenerate val + test probabilities on CPU
  ...
  3. Cache all prob vectors to phase4_final/probs_cache/ for the G5 paired bootstrap.
...
Run from the repo root:  python gru/phase4_final/05_gru_test_eval.py
"""
import csv
...
common = _load("gru_common", ROOT / "f1_optimization" / "00_common.py")
...
# arm -> (run subdir name, gru cfg used to build the model)
ARMS = {
    "gru_f1_winner": F1_WINNER_CFG,
    "gru_default_f1": DEFAULT_CFG,
    "gru_default_auc": DEFAULT_CFG,
}
...
def eval_arm(arm, cfg, Xva, yva, Xte, yte):
    ...
    rows = []
    for seed in SEEDS:
        model = engine.MODEL_REGISTRY["gru"](cfg)
...
def main():
    ...
    results = {}
    print("=== GRU arms — test set03 (single touch) ===")
    ...
    out = dict(parity_gate_max_delta=parity_max, seeds=SEEDS, arms=results)
...
def write_csv_md(results, parity_max):
    ...
    L = ["# GRU study — Phase G4 final test evaluation (set03, touched ONCE)", "",
         f"Single test pass through `05_gru_test_eval.py` after the G3 sign-off. "
    ...
    L += ["", "Reference (frozen, for orientation — formal deltas in Phase G5 "
