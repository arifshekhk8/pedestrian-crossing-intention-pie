"""01_sanity_checks.py — GRU study Phase G1 sanity gates (local CPU).

Mirrors transformer/phase1_setup/01_sanity_checks.py's discipline: every gate must pass
before any search is run. NO training-engine code is duplicated — everything routes through
journal_prep/issue12_unified_pipeline/12_unified_engine.py (the one unified engine), which is
the entire point of this study.
...
Gates:
  ...
  G1  param count             gru default (h128/nl2/do0.3) = 446,081 EXACTLY; print the search
                              param ladder (hidden {64,128,256} x num_layers {1,2}).
  G2  forward/backward CPU    output (B,1), no NaN, one BCE+Adam step runs; loss finite.
  G3  determinism (the load-bearing gate)  gru default seed 42 trained TWICE on CPU ->
...
Run from the repo root:  python gru/phase1_setup/01_sanity_checks.py
"""
...
ISSUE8_PATH = ROOT / "journal_prep" / "issue8_grid_search" / "08_grid_search.py"
...
engine = _load("gru_engine", ENGINE_PATH)
issue8 = _load("gru_issue8", ISSUE8_PATH)
...
def gate1(lines):
    p_default = n_params("gru", GRU_DEFAULT)
    ok = (p_default == EXPECTED_GRU_PARAMS)
    lines += [
        "## Gate 1 — parameter count & search ladder",
        "",
        f"- GRU default (h128 / nl2 / do0.3) = **{p_default:,}** params "
        f"(expect {EXPECTED_GRU_PARAMS:,})",
        "",
    ...
    for hidden in (64, 128, 256):
        for nl in (1, 2):
            ...
            lines.append(f"| {hidden} | {nl} | {n_params('gru', cfg):,} |")
...
def gate2(lines, data):
    engine.set_seed(42)
    model = engine.MODEL_REGISTRY["gru"](GRU_DEFAULT).to(CPU)
...
def _train_twice(family, cfg, data, lines, label):
...
def gate3(lines, data):
    r1, r2, deltas, identical = _train_twice("gru", GRU_DEFAULT, data, lines, "gru")
    lines += [
...
def gate4(lines, data):
    ...
    r1, r2, deltas, identical = _train_twice("bilstm", BILSTM_BASE, data, lines, "bilstm")
    ...
    lines += [
        ...
        "the GRU study rides the same rails.",
        "",
    ]
    return ok
...
def main():
    print("GRU G1 sanity gates (CPU) — this runs 4 short trainings, ~1-2 min...")
    lines = ["# GRU study — Phase G1 sanity report", "",
             ...
             "is duplicated. Generated by `gru/phase1_setup/01_sanity_checks.py`.", ""]

    ok0, data = gate0(lines); print(f"  Gate 0 (protocol asserts): {'PASS' if ok0 else 'FAIL'}")
    ok1 = gate1(lines);       print(f"  Gate 1 (param count 446,081): {'PASS' if ok1 else 'FAIL'}")
    ...
    lines += ["---", "",
              ...
              f"| G1 | GRU param count = 446,081 | {'PASS' if ok1 else 'FAIL'} |",
