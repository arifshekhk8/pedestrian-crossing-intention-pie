"""
04b_train_bbox_only.py — Day 6: bbox-only ablation (4D input, no ego-speed).

Comparison axis 2 from THESIS_PLAN.md:
  Bbox-only  vs  Bbox + ego-vehicle speed

Only difference from 04_train_bilstm.py:
  - Drops column index 4 (vehicle_speed) after loading → 4D input
  - Uses BiLSTMIntentPredictorFlex(input_dim=4)
  - Default out_dir: paper_and_artifacts/runs/bilstm_bbox_only
...
BiLSTMFlex = import_module("03b_bilstm_model_flex").BiLSTMIntentPredictorFlex
...
def evaluate(model, loader, device, threshold: float = 0.5):
    ...
    for xb, yb in loader:
        ...
        all_probs.append(probs); all_labels.append(yb.cpu().numpy())
        n += yb.size(0)
...
def main():
    ...
    ap.add_argument("--out_dir",      type=Path, default=Path("paper_and_artifacts/runs/bilstm_bbox_only"))
    ...
    print(f"input        : bbox-only 4D (ego-speed column dropped)")

    Xtr, ytr, Xva, yva, Xte, yte = load_splits(args.seq_dir)

    # ── KEY DIFFERENCE: drop ego-speed (column index 4) ──────────────────────
    # Original X shape: (N, 16, 5)  →  after drop: (N, 16, 4)
    # Columns kept: x1, y1, x2, y2
    # Column removed: vehicle_speed (index 4)
    Xtr = Xtr[:, :, :4]
    Xva = Xva[:, :, :4]
    Xte = Xte[:, :, :4]
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # Normalize using train-only stats (computed on 4D data)
    ...
    # input_dim=4 because we dropped ego-speed
    model     = BiLSTMFlex(input_dim=4).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss.backward()
            optimizer.step()
            running_loss += loss.item() * yb.size(0); n += yb.size(0)
        ...
        if val["auc"] > best_auc:
            best_auc = val["auc"]; epochs_no_improve = 0
            torch.save({
    ...
    # ── Test eval on best val checkpoint ─────────────────────────────────────
