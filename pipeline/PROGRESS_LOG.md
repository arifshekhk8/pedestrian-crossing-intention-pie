...
- **Vanilla-RNN study (`rnn/`, 2026-07-14).** The gating isolation: only `nn.LSTM`→`nn.RNN`,
  so the cell loses its gates entirely. Same Issue-8 search + F1-first optimization on the
  same engine. **TIES BiLSTM-F1 and GRU-F1 on F1** (ΔF1 +0.0033 / −0.0038, CIs include 0),
  level-to-marginally-better than the frozen BiLSTM on AUC at matched h128 (ΔAUC +0.0059),
  and — unlike the GRU — its AUC-optimised version **ties the searched transformer on AUC**
  (ΔAUC −0.0013, CI includes 0). RNN-F1 test F1 0.852 / AUC 0.948 / Acc 0.902; smallest and
  fastest family at 0.316 ms/window CPU; 0 diverged runs. **Not even gating matters over a
  16-step window.** Full: `rnn/SUPERVISOR_SUMMARY.md`.
- **Cross-study analysis tables (`journal_prep/Analysis/`, 2026-07-19).** Every model from
  all four families in one table, plus latency and hyperparameter tables, all regenerated by
  `00_generate_analysis.py`. This is now the single place to read results from.
- **Observation-window extension (`journal_prep/obs_window_extension/`, 2026-07-19).**
  Supervisor directive: test whether the equivalence survives a longer history. The
  F1-optimised model of each family re-run at OW32 and OW64. **F1 declines with window
  length for every family**; the four still tie at OW16 and OW32 (spread ≤ 0.004 at OW32,
  inside per-seed noise); **at OW64 the un-gated vanilla RNN alone falls behind** (F1 0.802,
  a drop about twice that of the gated cells). Read within a window, not across: the test
  cohort shrinks 2094 → 1009 → 458 as the window grows, so cross-window absolutes are not
  comparable. **The equivalence result is horizon-bounded, and now measured.**
- **JAAD cross-dataset validation (`journal_prep/cross_dataset_validation/`, 2026-07-21).**
  Event-anchored extraction, leakage audit and the four-family comparison replicated on
  JAAD through the same unified engine. Tests the leakage and architecture claims out of
  domain; **cannot** test the input claim, because JAAD carries no ego-vehicle speed. The
  JAAD dataset is not vendored here. Method and results: `cross_dataset_validation/PLAN.md`.
- **IDD-PeD cross-dataset study (`idd_ped_crossdataset/`, 2026-08).** IDD-PeD does carry
  per-frame OBD speed in PIE's km/h scale, so this is the first out-of-domain test of the
  actual 5-D input contract, in unstructured South-Asian traffic. Two experiments:
  zero-shot transfer of the frozen PIE checkpoints (4 families × 5 seeds, PIE normalization,
  no fine-tuning or threshold tuning) and an independent from-scratch replication on
  IDD-PeD alone. Self-contained folder; dataset and checkpoints are gitignored. See
  `idd_ped_crossdataset/README.md`.
- **Manuscript (`paper_and_artifacts/Journal_writing/submission/`, 2026-08).** MDPI MTI
  submission package written from scratch against the template: seven sections, external
  bibliography, and an `evidence/` folder holding the verification behind the text —
  baseline numbers read from the primary PDFs, method warrants including the two the
  literature contradicts, a number-by-number audit, and sources for the road-safety
  statistics. Figure generators for both Introduction figures live in `evidence/`.
